...
class Cms:
    ...
    def login(self):
        ...
        url = 'http://%s:8088/user/login' % self.ip
        host = '%s:8088' % self.ip
        referer = 'http://%s:8088/user/login' % self.ip
        headers = {
            ...
            # 'DNT': '1',
            'origin': 'http://%s:8088' % self.ip,
            # 'Cookie': self.cookie
        }
        login_url = 'http://%s:8088/cdn/submitLogin' % self.ip
        ...
        # 注意此时用的json参数
        res = self.session.post(url=login_url, json=login_post, headers=self.headers)
    ...
    # get all channel
    def get_all(self):
        url = 'http://{}:8088/cdn/record?current=1&pageSize=10000'.format(self.ip)
        res = self.session.get(url=url, headers=self.headers)
        # print(res.text)
        return res.text

    # get channels status in batch
    def select_batch(self, state, origin, pagesize=5000):
        url = 'http://{}:8088/cdn/record?seconds=300&state={}&origin={}&current=1&pageSize={}'.format(self.ip, state, origin, pagesize)
        res = self.session.get(url=url, headers=self.headers)
        print(res.text)
        return res.text
